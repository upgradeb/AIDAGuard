...
版本 0.3.0 — 7 个 crate 的工作空间 + Tauri 桌面前端。
...
┌────────────────────────────────────────────────┐
│                Tauri 桌面应用                   │
│  React/TypeScript + Ant Design + i18n (en/zh) │
├────────────────────────────────────────────────┤
│      Tauri 命令层（Rust 后端桥接）              │
├───────────────┬────────────────┬───────────────┤
│  aidaguard-   │  aidaguard-    │  aidaguard-    │
│   plugins     │   proxy        │   upstream     │
│   插件系统     │   代理服务      │   上游管理      │
├───────────────┴────────────────┴───────────────┤
│         aidaguard-detector                     │
│   模式识别器 + NLP NER + 匿名化器               │
├────────────────────────────────────────────────┤
│         aidaguard-core                         │
│   配置、实体类型、DetectionEngine 接口、        │
│   检测器（正则引擎）、替换器                     │
├────────────────────────────────────────────────┤
│         aidaguard-storage                      │
│   SQLite + AES-256-GCM 加密审计日志             │
└────────────────────────────────────────────────┘
```
...
│   ├── useConfigStore.ts、useProxyStore.ts、useRulesStore.ts、
│   ├── useAuditStore.ts、useToolsStore.ts、useUpstreamStore.ts、
│   └── useThemeStore.ts
├── i18n/
│   ├── index.ts             → i18next 配置（en/zh、语言检测）
│   ├── en.ts                → 英文翻译（约 274 个 key）
│   └── zh.ts                → 中文翻译（约 274 个 key）
├── pages/
│   ├── Dashboard.tsx        → 代理控制、统计卡片、事件推送、操作指引
│   ├── Settings.tsx         → 代理、检测策略（地区/行业）、NLP、存储、日志、通知、外观、关于
│   ├── Rules.tsx            → 规则管理：分类、CRUD、测试面板、大模型生成规则
│   ├── AuditLog.tsx         → 审计表格、过滤器、详情面板、分组视图、统计、导出
│   ├── ToolsConfig.tsx      → 插件列表：检测、配置、还原 AI 工具
│   └── Upstreams.tsx        → LLM 提供商管理：增/改/删、设为默认、测试连通性
├── components/
│   ├── StatCard.tsx         → 指标卡片（今日/总计/本周计数）
│   ├── EventFeed.tsx        → 实时检测事件列表
│   ├── AuditTable.tsx       → 分页审计记录表格
│   ├── AuditDetailPanel.tsx → 完整审计记录详情（原文 vs 脱敏后）
│   ├── RuleEditor.tsx       → 规则创建/编辑表单（id、名称、正则、排除、策略、模式等）
│   ├── RuleTestPanel.tsx    → 测试文本输入 + 匹配结果展示
│   ├── RuleHitChart.tsx     → 规则命中分布可视化
│   ├── GenerateRuleModal.tsx→ 基于示例文本的大模型规则生成
│   ├── OperationGuide.tsx   → 新用户逐步设置引导
...
**UI 页面概览：**
1. **Dashboard（仪表盘）** — 代理启停、实时统计（今日/总计/本周）、事件推送、操作指引
2. **Settings（设置）** — 代理端口/规则目录/最大请求体、地区/行业检测策略、NLP NER 设置、存储数据库、日志、通知、主题
3. **Rules（规则管理）** — 规则分类（创建/重命名/删除）、规则 CRUD、测试面板、通过大模型生成规则
4. **AuditLog（审计日志）** — 过滤/分组审计记录、详情面板（原始 vs 脱敏文本）、CSV/JSON 导出
5. **ToolsConfig（工具配置）** — AI 编程工具检测、代理配置、备份/还原、启用/禁用插件
