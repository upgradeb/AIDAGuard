...
| Crate | Purpose |
|-------|---------|
| `aidaguard-core` | Core types (`EntityType`, `Config`), traits (`DetectionEngine`), YAML rule detector, replacer |
...
### 1. YAML Rules (`aidaguard-core/src/detector/`)

- Loaded from `rules/` directory (hot-reloadable)
- `RuleDef`: id, name, pattern, exclude, enabled, strategy, mode, priority, compliance
- Simple regex matching without validation or context scoring

### 2. Pattern Recognizers (`aidaguard-detector/src/recognizers/pattern/`)

- Hardcoded Rust code with:
...
### Unified Pipeline (`AnalyzerEngine`)

```rust
// In aidaguard-detector/src/pipeline.rs
// Merges both systems: YAML rules + pattern recognizers
